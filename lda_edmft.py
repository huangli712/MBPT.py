...
class LocVar(object):
    ...
    def sym_ploc(self, sy_pm):
        ...
        for orb in range(self.__nband):
            ...
            fit = scipy.interpolate.InterpolatedUnivariateSpline(bmesh[12:48], self.__ploc[orb,12:48].real, k = 2)
            self.__ploc[orb,0:12] = fit( bmesh[0:12] ) + 1j * self.__ploc[orb,0:12].imag
...
if __name__ == '__main__':
    ...
    #---------------------------------------------------------------------
        ...
        if ( g_conv or s_conv or p_conv ) and _iter_ > 10 :
            if pyalps.mpi.rank == 0:
